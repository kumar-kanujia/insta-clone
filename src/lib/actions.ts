...
export async function deletePost(formData: FormData) {
  ...
  try {
    ...
    utapi.deleteFiles(removeUrlPrefix(post.fileUrl));
...
export async function bookmarkPost(value: FormDataEntryValue | null) {
  ...
  const bookmark = await prismadb.savedPost.findUnique({
    where: {
      postId_userId: {
        postId,
        userId,
  ...
  if (bookmark) {
    try {
      await prismadb.savedPost.delete({
        where: {
          postId_userId: {
            postId,
            userId,
...
export async function followUser(formData: FormData) {
  ...
  const follows = await prismadb.follows.findUnique({
    where: {
